...
def RUNCMD(var, command_name):
    ...
    command_pair = ED9InstructionsSet.reverse_commands_dict.get(command_name)
    if command_pair is None:
        parts = command_name.split("_") if isinstance(command_name, str) else []
        if (
            len(parts) == 4
            and parts[0] == "Cmd"
            and parts[1] == "unknown"
            ...
            try:
                command_pair = (int(parts[2], 16), int(parts[3], 16))
            except ValueError:
                command_pair = None
            if command_pair is not None and not all(
                0 <= value <= 0xFF for value in command_pair
            ):
                command_pair = None
        if command_pair is None:
            raise KeyError(command_name)
    (id_struct, op_code) = command_pair
