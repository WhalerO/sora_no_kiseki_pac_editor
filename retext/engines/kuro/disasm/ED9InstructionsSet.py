...
def OP_24(instr, stream):
    ...
    command_pair = (structID, command_op_code)
    command_name = commands_dict.get(command_pair)
    if command_name is None:
        # OP_24 stores the numeric pair in the bytecode.  A missing friendly
        # name is therefore not an unknown binary layout and must not prevent
        # the rest of the script (including PUSHSTRING text) from being read.
        # Keep the numeric identity in a deterministic assembler-readable
        # placeholder instead of guessing a semantic command name.
        command_name = f"Cmd_unknown_{structID:02X}_{command_op_code:02X}"
        commands_dict[command_pair] = command_name
        reverse_commands_dict[command_name] = command_pair

    instr.operands.append(operand(nb_args, False)) 
    instr.operands.append(operand(command_name, False)) #The user needs the number of arguments since it's not really apparent for the moment
...
class instruction(object):
    ...
    def to_string(self, stream)->str:
        ...
        for operand_id in range(len(self.operands)-1):
            ...
            if (type(value) == str):
                result = result + json.dumps(value, ensure_ascii=False)
            else:
